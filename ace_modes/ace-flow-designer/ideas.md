# ace-flow-designer - ideas / scratchpad
...
"<Name> - flow-builder start prompt"). ace-flow-designer is the interactive skill that *produces* that
start prompt - aimed at a junior developer who knows the goal but not how to spec it.

Pipeline: requirement → **ace-flow-designer (interview)** → start prompt → ace-flow-builder → files.

## Design decisions (v0.1.0)

- Sibling of ace-flow-builder under `ace/`, not a variant - it has its own distinct workflow.
- Reuses `../ace-flow-builder/references/ace_patterns_catalog.md` at D1 (no-duplication rule).
- Output = `<FlowName>_start_prompt.md`; H1 mirrors the worked exemplar.
- State file `<FlowName>.design_state.md` mirrors ace-flow-builder's `flow_state.md` so interviews resume.
...
- The output explicitly pre-answers ace-flow-builder's Phase B1 so the build starts without re-asking.

## Design decisions (v0.2.0 - flow-builder contract alignment)

Cross-referenced ace-flow-designer's output against flow-builder's full input contract (extracted via
agents from ace-flow-builder's workflow.md / restapi_build.md / validated_rules.md) and the gold-standard
...
- [ ] Decide whether ace-flow-builder's `example/input.md` should point at this skill's output format.
