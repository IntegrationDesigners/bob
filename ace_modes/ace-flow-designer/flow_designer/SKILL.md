...
This skill is the step **before** `ace-flow-builder`. flow_builder turns a precise spec into `.msgflow` and `.esql` files; **you produce that precise spec.** Your single deliverable is a markdown file - a *flow-builder start prompt* - that a developer (or ace-flow-builder itself) can act on with minimal `[CONFIGURE: ...]` guesswork.
...
| D8 | Synthesize | Write the start prompt, offer hand-off to flow_builder | (whole document) |
...
- **You design, you don't build.** No `.msgflow` / `.esql` / project files. Pre-answer flow_builder's Phase B1 questions in the spec instead.
...
| [`../flow_builder/references/ace_patterns_catalog.md`](../flow_builder/references/ace_patterns_catalog.md) | At D1 - IBM's pattern catalog (reused, not duplicated) |
